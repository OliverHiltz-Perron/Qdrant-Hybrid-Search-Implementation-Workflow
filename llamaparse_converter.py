...
def process_and_save(documents, output_dir):
    ...
    concatenates, and saves the full markdown content to the output directory as soon as each file is ready.
    """
    if not documents:
    ...
    print("Grouping content chunks by target output file and saving as soon as each file is ready...")
    for i, doc in enumerate(documents):
        try:
            ...
            # Check if this is the last chunk for this file (by counting all chunks for this file)
            # Since we don't know chunking in advance, we save after each chunk for now
            # To avoid partial files, let's save after every chunk, but overwrite the file each time
            full_markdown_content = "\n\n".join(grouped_content[output_filepath])
            print(f"Writing (so far) concatenated content to '{output_filepath}' after chunk {i+1}...")
            with open(output_filepath, 'w', encoding='utf-8') as f:
                f.write(full_markdown_content)
            print(f"Saved '{output_filepath}' after chunk {i+1}.")
    ...
    print(f"Finished processing. Saved {len(grouped_content)} markdown files to '{output_dir}'.")
